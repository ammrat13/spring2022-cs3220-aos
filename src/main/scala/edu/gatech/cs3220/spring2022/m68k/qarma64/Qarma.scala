...
object Qarma {
  ...
  val REFLECTION_CONSTANT: Block =
    new Block(Seq(0xc0, 0xac, 0x29, 0xb7, 0xc9, 0x7c, 0x50, 0xdd).map(new Cell(_)))

  /** Default values for round keys See: https://eprint.iacr.org/2016/444.pdf
    * Table 1
    */
  val ROUND_KEYS: Seq[Block] = Seq(
    new Block(Seq(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00).map(new Cell(_))),
    new Block(Seq(0x13, 0x19, 0x8a, 0x2e, 0x03, 0x70, 0x73, 0x44).map(new Cell(_))),
    new Block(Seq(0xa4, 0x09, 0x38, 0x22, 0x29, 0x9f, 0x31, 0xd0).map(new Cell(_))),
    new Block(Seq(0x08, 0x2e, 0xfa, 0x98, 0xec, 0x4e, 0x6c, 0x89).map(new Cell(_))),
    new Block(Seq(0x45, 0x28, 0x21, 0xe6, 0x38, 0xd0, 0x13, 0x77).map(new Cell(_))),
    new Block(Seq(0xbe, 0x54, 0x66, 0xcf, 0x34, 0xe9, 0x0c, 0x6c).map(new Cell(_))),
    new Block(Seq(0x3f, 0x84, 0xd5, 0xb5, 0xb5, 0x47, 0x09, 0x17).map(new Cell(_))),
    new Block(Seq(0x92, 0x16, 0xd5, 0xd9, 0x89, 0x79, 0xfb, 0x1b).map(new Cell(_)))
  )
}
